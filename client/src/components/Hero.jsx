...
const Hero = () => {
    ...
    const handleSearch = (e)=>{
        e.preventDefault()
        navigate(ROUTES.cars + '?pickupLocation=' + pickupLocation + '&pickupDate=' + pickupDate + '&returnDate=' + returnDate)
    }
  ...
  return (
    ...
    className='h-screen flex flex-col items-center justify-center gap-14 bg-light text-center'>
        ...
        className='text-4xl md:text-5xl font-semibold'>Luxusautos zur Miete</motion.h1>
       ...
       onSubmit={handleSearch} className='flex flex-col md:flex-row items-start md:items-center justify-between p-6 rounded-lg md:rounded-full w-full max-w-80 md:max-w-200 bg-white shadow-[0px_8px_20px_rgba(0,0,0,0.1)]'>
        ...
        <div className='flex flex-col md:flex-row items-start md:items-center gap-10 min-md:ml-8'>
            <div className='flex flex-col items-start gap-2'>
                ...
                <p className='px-1 text-sm text-gray-500'>{pickupLocation ? pickupLocation : 'Bitte waehle einen Ort aus'}</p>
            </div>
            <div className='flex flex-col items-start gap-2'>
